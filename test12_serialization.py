...
#	可以随时修改变量，比如把name改成'Bill'，但是一旦程序结束
#	变量所占用的内存就被操作系统全部回收
...
#	(file-like Object 按照字面解释就是类文件对象)
f = open('dump.txt', 'wb')
pickle.dump(d, f)
f.close()
...
d = pickle.load(f)
f.close()
#	再在命令行中Python交互中输入 d，就能得到我们之前输入的 {name = 'Bob', age = 20, score = 90}
#	当然，这个变量和原来的变量是完全不相干的对象，它们只是内容相同而已


#	ickle的问题和所有其他编程语言特有的序列化问题一样，就是它只能用于Python
...
print(jsaon.dumps(s, default = studentToDict))
#		返回：
